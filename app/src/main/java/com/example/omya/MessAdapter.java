...
public class MessAdapter extends RecyclerView.Adapter<MessAdapter.ViewHolderMess> {
    private ArrayList<MessModel> mess_list;
    private Context mess_context;

    public MessAdapter(ArrayList<MessModel> mess_list, Context mess_context){
    ...
    @NonNull
    @Override
    public MessAdapter.ViewHolderMess onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(mess_context).inflate(R.layout.owner_mess_look, parent, false);
        return new MessAdapter.ViewHolderMess(view);

    }

    @Override
    public void onBindViewHolder(@NonNull MessAdapter.ViewHolderMess holder, int position) {
